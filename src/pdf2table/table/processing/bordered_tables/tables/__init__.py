...
from pdf2table.table.processing.bordered_tables.tables.cell_clustering import cluster_cells_in_tables
from pdf2table.table.processing.bordered_tables.tables.semi_bordered import add_semi_bordered_cells
from pdf2table.table.processing.bordered_tables.tables.table_creation import cluster_to_table, normalize_table_cells
...
def get_tables(cells: List[Cell], elements: List[Cell], lines: List[Line], char_length: float) -> List[Table]:
    ...
    return [tb for tb in tables if tb.nb_rows * tb.nb_columns >= 4]
